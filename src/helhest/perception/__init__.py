"""Perception stack: point cloud -> heightmap -> traversability cost map, plus the
lidar sim, outlier/dynamic filters, trust masks, and the rolling map accumulator.

The perception->planning seam lives here too (`gridmap.GridMap` contract,
`grid_adapter.grid_params_from`, `terrain_lidar`). Localization (ICP-based pose
estimation) is a sibling package, `helhest.localization`.
"""
