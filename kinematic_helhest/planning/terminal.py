...
def dock_control(state, goal, wmax=4.0, slow_radius=1.0, turn_gain=3.0, turn_width=0.5):
    """state (x, y, yaw), goal (x, y) -> wheel command (wL, wR, rear) for one step.

    slow_radius: distance over which to decelerate to a stop. turn_gain/turn_width: how hard to
    steer toward the goal (differential per rad of heading error)."""
    ...
    v = wmax * min(1.0, dist / slow_radius)              # decelerate as the goal nears
